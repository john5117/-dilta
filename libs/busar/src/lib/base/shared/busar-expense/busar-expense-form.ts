...
export class BusarExpenseFormBase implements OnInit {
  ...
  @Input() public busar = ({} as any) as Collector;
  ...
  public form(expense?: Expense) {
    ...
    if (!expense) {
      expense = defaultKeys(expense, expenseKeys);
    }
  ...
  public emit(envt: Expense) {
    envt.busarId = this.busar.universalId || this.expense.busarId;
  ...
  public errorDetector() {
    ...
    if (this.expense) {
      errorInvalid(
        getProp(this.expense, 'busarId') &&
          getProp(this.expense, 'name'),
